...
Tin nhắn mới trong conversation mà mình là participant. **Nhận cho MỌI tin nhắn — kể cả tin do chính mình gửi** (mọi thiết bị của bạn đều nhận).

> **Contract quan trọng: WS là nguồn render duy nhất.** FE **KHÔNG** append tin nhắn từ response của `POST /conversations/messages` (chỉ coi response đó là ack — lấy `messageId`/status). Chỉ append tin trong handler `message.new`. Mỗi tin đến **đúng 1 lần** qua WS → không cần dedupe. Cách này giữ mọi thiết bị đồng bộ (multi-device).
...
interface MessageNewPayload {
  ...
  replyId?: string;
  replySnippet?: string;     // 80 ký tự đầu của tin được reply ('[Hình ảnh]' nếu reply ảnh)
...
socket.on('message.new', (payload: MessageNewPayload) => {
  ...
  // Kể cả khi payload.senderId === currentUserId (tin của chính mình từ thiết bị khác/này).
});
```
...
export function ChatPage({ conversationId }: { conversationId: string }) {
  ...
  // NGUỒN RENDER DUY NHẤT: chỉ append từ message.new (kể cả tin của chính mình).
  ...
  async function handleSend(content: string) {
    // Gửi qua REST — KHÔNG append kết quả vào messages ở đây.
    // Tin sẽ tự về qua message.new và được append ở handler trên (tránh trùng + đồng bộ đa thiết bị).
    await api.post('/conversations/messages', { conversationId, content });
  }
...
A: **Gửi qua REST** (`POST /conversations/messages`), **nhận/render qua WS** (`message.new`). REST response chỉ là ack — đừng append nó vào UI; tin sẽ tự về qua `message.new`.

**Q: Sender có nhận `message.new` của tin mình vừa gửi không?**
A: **Có** — mọi thiết bị của sender đều nhận (contract mới). FE render tin từ `message.new`, KHÔNG append từ REST response → tránh trùng và giữ các thiết bị đồng bộ (multi-device). Mỗi tin đến đúng 1 lần nên không cần dedupe.
