...
jest.mock('web-push', () => ({ sendNotification: jest.fn().mockResolvedValue(undefined), setVapidDetails: jest.fn() }));
...
describe('NotificationService push payload', () => {
  ...
  const service = new NotificationService(
    {} as ConfigService,
    { findById: jest.fn(async (id: string) => users[id] ?? null), removePushSubscription: jest.fn() } as unknown as IUserRepository,
    { findById: jest.fn(async () => ({ participants: [{ userId: 'alice' }, { userId: 'bob' }] })) } as unknown as IConversationRepository,
    presence,
  );
  const sent = () => JSON.parse((webpush.sendNotification as jest.Mock).mock.calls.at(-1)[1]);

  beforeEach(() => (webpush.sendNotification as jest.Mock).mockClear());

  it('uses English text with the message content', async () => {
    await service.handleMessageSentEvent(new MessageSentEvent('c1', 'alice', 'm1', 'hello', 'p1'));
    expect(sent()).toEqual({ title: 'BabyChat', body: 'alice: hello', url: '/messages?c=c1' });
  });

  it('uses a generic English sentence when there is no text (image/sticker)', async () => {
    await service.handleMessageSentEvent(new MessageSentEvent('c1', 'alice', 'm2', '', 'p1'));
  ...
  it('does not push to a user who is viewing the app', async () => {
    ...
    await service.handleMessageSentEvent(new MessageSentEvent('c1', 'alice', 'm3', 'hi', 'p1'));
